...
export default function BoardListPage({ boardType }) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((e) =>
        setError(e.userMessage ?? "게시글 목록을 불러오지 못했습니다."),
      )
      .finally(() => setLoading(false));
  }, [boardType, keyword, category, mine, params]);
  function search(e) {
    e.preventDefault();
    const f = new FormData(e.currentTarget);
    setParams({
      ...(f.get("keyword") ? { keyword: f.get("keyword") } : {}),
      ...(f.get("category") ? { category: f.get("category") } : {}),
      ...(mine ? { mine: "true" } : {}),
    });
  }
      ...
      <div className="card overflow-hidden">
        ...
        >
          ...
          {user && (
            <button
              type="button"
              className={`btn-secondary ${mine ? "!border-[#ff512f] !text-[#ff512f]" : ""}`}
              onClick={() => setParams(mine ? {} : { mine: "true" })}
            >
              내 글
            </button>
          )}
            ...
            {data.content.map((p) => (
              <li key={p.postId}>
                ...
                >
                  ...
                  <span className="text-sm text-[#475467]">{p.authorName}</span>
                  <span className="text-xs text-[#667085]">
                    조회 {p.viewCount} · 좋아요 {p.likeCount} · 댓글{" "}
                    {p.commentCount}
                  </span>
                </Link>
              </li>
...
function State({ text, error }) {
  return (
    <div
      className={`p-16 text-center ${error ? "text-red-600" : "text-[#667085]"}`}
    >
      {text}
    </div>
  );
